...
function busyHolidays(shoppers, orders, leadTime) {
    ...
    let combs = getAllCombs( shoppersCanDoOrdersArr );
    console.log(combs)
...
function canFulfil(shoppersByOrder, count, leadTime) {
    let s;  
    for ( let shopper of shoppersByOrder) {
        if ( !shopper.length) continue;
        for ( let order of shopper) {
            s = shoppersByOrder.map( shopper => {
                let i = shopper.indexOf(c);
                if ( i !== -1) shopper.splice(i,1)
                return shopper;
            })
            count --;
    ...
    s.shift(); 
    if (canFulfil(s, count, leadTime) === true) return true;
    if ( count === 0) {}
}

function getNumShopperCanFulfilOrder(shoppersCanDoOrdersArr, count) {
    let shoppersByOrder = [];
    for( count; count --; ) {
        let dummy = [];
        shoppersCanDoOrdersArr.forEach( (shopper,i) => {
            if ( shopper.includes(count)) dummy.push(i)
        })
        shoppersByOrder.unshift(dummy);
    }
    return shoppersByOrder;
}
